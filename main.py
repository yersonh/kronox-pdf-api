...
from analyzer import analyze, analyze_acta, analyze_minuta, analyze_planilla, analyze_supervisor
from extractor import extract_text
from models import ActaResumenResponse, AnalysisResponse, ExtractResponse, MinutaAnalysisResponse, PlanillaAnalysisResponse, SupervisorAnalysisResponse
...
@app.post("/analyze-acta", response_model=ActaResumenResponse, dependencies=[Depends(verificar_api_key)])
async def analyze_acta_pdf(archivo: UploadFile = File(...)):
    """
    Specialized endpoint for meeting minutes (acta de reunión).
    Returns a two-paragraph executive summary generated by Gemini.
    """
    pdf_bytes = await _read_pdf(archivo)
    text, _, method = extract_text(pdf_bytes)
    return analyze_acta(pdf_bytes, text, method)
